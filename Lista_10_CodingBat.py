...
# near_ten(19) -> True
def near_ten(n):
  return n in range (8,11) or n % 10 <= 2 or n % 10 >= 8
...
def soma_na_lista(n, lista):
  for i in range (len(lista)):
    for j in range (len(lista)):
      if lista[i] + lista[j] == n and lista[i] != lista[j]: return True
  return False
...
def fila_tijolos(n_peq, n_gra, meta):
  if n_gra * 5  > meta and n_gra * 5  != meta: #verificando se a "meta" é completada (ou aproximadamente) pelos tijolos de tamanho 5.
    return n_peq >= meta % 5
  return n_gra * 5 + n_peq >= meta
